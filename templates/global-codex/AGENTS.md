...
Use filesystem tools only when you need the actual source lines of a specific file or function
...
An empty result from `codebase_graph_query` does not mean the code has no symbols. It may mean
the graph has not been built yet. Always check `graph_status()` first.
