...
type BrandMark = {
  ...
  path: string;
  /**
   * Monochrome marks (e.g. GitHub) that were shipped as white-only SVGs.
   * When true, fill uses `currentColor` so light theme keeps contrast.
   */
  adaptive?: boolean;
};
...
const BRAND: Record<string, BrandMark> = {
  github: {
    label: "GitHub",
    // Official Simple Icons / GitHub brand black; adaptive fill overrides in CSS
    color: "181717",
    adaptive: true,
...
function BrandSvg({
  ...
  const fill = brand.adaptive ? "currentColor" : `#${brand.color}`;
  return (
    <svg
      role="img"
      viewBox="0 0 24 24"
  ...
  if (brand) {
    return (
      <span
        className={[
          "connector-icon",
          "connector-icon--brand",
          brand.adaptive ? "connector-icon--brand-adaptive" : "",
          className,
        ]
          .filter(Boolean)
          .join(" ")}
