...
export async function upsertFindingAcrossSessions(
  ...
  if (USER_CLOSED.includes(existing.status)) {
    ...
    const next = await store.update(existing.id, {
      ...
      modelConfidence: candidate.modelConfidence ?? existing.modelConfidence,
      tokenConfidence: candidate.tokenConfidence ?? existing.tokenConfidence,
      suggestion: candidate.suggestion ?? existing.suggestion,
      suggestedFix: candidate.suggestedFix ?? existing.suggestedFix,
      existingCode: candidate.existingCode ?? existing.existingCode,
      reasoning: candidate.reasoning ?? existing.reasoning,
