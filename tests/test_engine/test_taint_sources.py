...
class TestCTaintSources:
    ...
    def test_cgi_getenv_query_string_emits_source(self, parser) -> None:
    ...
    def test_cgi_getenv_http_cookie_emits_source(self, parser) -> None:
    ...
    def test_fread_stdin_emits_source(self, parser) -> None:
        """fread(buf, 1, n, stdin) produces a taint-source node."""
        result = parser.parse("main.c", _C_CGI_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("stdin" in n for n in names)

    def test_calls_edges_point_at_handler(self, parser) -> None:
    ...
    def test_mongoose_functions_emit_source(self, parser) -> None:
    ...
    def test_non_http_getenv_no_source(self, parser) -> None:
        """getenv('PATH') must NOT produce a taint-source node."""
        result = parser.parse("util.c", _C_NO_HTTP_SOURCE, "t", "r")
        external = [n for n in result.nodes if n.node_type == "external"]
        assert len(external) == 0

    def test_source_node_file_is_c_http(self, parser) -> None:
    ...
    def test_no_duplicate_source_nodes(self, parser) -> None:
...
class TestCppTaintSources:
    ...
    def test_drogon_getbody_emits_source(self, parser) -> None:
    ...
    def test_drogon_get_parameter_emits_source(self, parser) -> None:
        """req->getParameter() triggers a taint-source node."""
        result = parser.parse("handler.cpp", _CPP_DROGON_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("getParameter" in n for n in names)

    def test_crow_body_field_emits_source(self, parser) -> None:
    ...
    def test_cpp_cgi_getenv_emits_source(self, parser) -> None:
        """getenv('QUERY_STRING') in C++ file also emits a taint source."""
        result = parser.parse("cgi.cpp", _CPP_CGI_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("QUERY_STRING" in n for n in names)

    def test_calls_edge_points_at_handler(self, parser) -> None:
    ...
    def test_source_node_file_is_cpp_http(self, parser) -> None:
...
class TestCSharpTaintSources:
    ...
    def test_from_query_attr_emits_source(self, parser) -> None:
    ...
    def test_from_body_attr_emits_source(self, parser) -> None:
        """[FromBody] parameter emits a taint-source."""
        result = parser.parse("UserController.cs", _CS_PARAM_ATTRS_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("FromBody" in n for n in names)

    def test_from_form_attr_emits_source(self, parser) -> None:
        """[FromForm] parameter emits a taint-source."""
        result = parser.parse("UserController.cs", _CS_PARAM_ATTRS_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("FromForm" in n for n in names)

    def test_request_query_member_emits_source(self, parser) -> None:
    ...
    def test_source_node_file_is_aspnetcore(self, parser) -> None:
    ...
    def test_calls_edges_emitted(self, parser) -> None:
...
class TestRustTaintSources:
    ...
    def test_actix_path_extractor_emits_source(self, parser) -> None:
...
class TestPhpTaintSources:
    ...
    def test_get_superglobal_emits_source(self, parser) -> None:
    ...
    def test_post_superglobal_emits_source(self, parser) -> None:
        """$_POST access emits a taint-source node."""
        result = parser.parse("index.php", _PHP_SUPERGLOBAL_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("POST" in n for n in names)

    def test_cookie_superglobal_emits_source(self, parser) -> None:
        """$_COOKIE access emits a taint-source node."""
        result = parser.parse("index.php", _PHP_SUPERGLOBAL_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("COOKIE" in n for n in names)

    def test_laravel_input_method_emits_source(self, parser) -> None:
    ...
    def test_source_node_file_is_php(self, parser) -> None:
...
class TestKotlinTaintSources:
    ...
    def test_spring_request_param_emits_source(self, parser) -> None:
    ...
    def test_spring_request_body_emits_source(self, parser) -> None:
        """@RequestBody annotation emits a taint-source node."""
        result = parser.parse("UserController.kt", _KOTLIN_SPRING_SOURCE, "t", "r")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("RequestBody" in n for n in names)

    def test_source_node_file_is_kotlin_http(self, parser) -> None:
...
class TestScalaTaintSources:
    ...
    def test_play_query_string_emits_source(self, parser) -> None:
    ...
    def test_akka_parameters_directive_emits_source(self, parser) -> None:
    ...
    def test_source_node_file_is_scala_http(self, parser) -> None:
...
class TestTypeScriptNestJSTaintSources:
    ...
    def test_param_decorator_emits_source(self, parser) -> None:
    ...
    def test_query_decorator_emits_source(self, parser) -> None:
        """@Query() decorator emits a nestjs.Query taint-source node."""
        result = parser.parse("user.controller.ts", _TS_NESTJS_PARAM_SOURCE, "t", "r", "typescript")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("Query" in n for n in names)

    def test_body_decorator_emits_source(self, parser) -> None:
        """@Body() decorator emits a nestjs.Body taint-source node."""
        result = parser.parse("user.controller.ts", _TS_NESTJS_PARAM_SOURCE, "t", "r", "typescript")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("Body" in n for n in names)

    def test_headers_decorator_emits_source(self, parser) -> None:
        """@Headers() decorator emits a nestjs.Headers taint-source node."""
        result = parser.parse("user.controller.ts", _TS_NESTJS_PARAM_SOURCE, "t", "r", "typescript")
        names = {n.name for n in result.nodes if n.node_type == "external"}
        assert any("Headers" in n for n in names)

    def test_source_node_file_is_nestjs(self, parser) -> None:
