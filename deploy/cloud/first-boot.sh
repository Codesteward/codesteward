...
#   IMAGE_TAG       App/UI image tag (default from VERSION file or 1.3.0)
...
IMAGE_TAG="${IMAGE_TAG:-$(tr -d '[:space:]' <"$VERSION_FILE" 2>/dev/null || echo 1.3.0)}"
...
main() {
  ...
  case "${IMAGE_TAG:-}" in
    *'${'*|*imageTag*) IMAGE_TAG="$(tr -d '[:space:]' <"$VERSION_FILE" 2>/dev/null || echo 1.3.0)"; log "Using IMAGE_TAG=$IMAGE_TAG" ;;
  esac
  case "${DOMAIN:-}" in
    *'${'*|domain) DOMAIN="" ;;
  esac
